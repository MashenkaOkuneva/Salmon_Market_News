...
class RawDocs():
    ...
    def __init__(self, doc_data, stopwords=set(), contraction_split=True, collocations = True):
...
must be passed to RawDocs")
        ...
        self.journalist_names = preprocess_data.journalist_names
    ...
    def token_clean(self, length, numbers=True):
        ...
        def clean1(tokens):
            return [t for t in tokens if (t.isalpha() == 1 and len(t) > length) or (u'_' in t)] 
        def clean2(tokens):
